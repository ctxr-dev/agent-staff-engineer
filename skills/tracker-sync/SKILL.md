...
- **Linear**: `issues.*` and `labels.*` fully implemented via Linear's GraphQL API (direct `fetch`, no `@linear/sdk`). Auth via `LINEAR_API_KEY` env var. `review.*` and `projects.*` stubbed with pointed messages (Linear has no native PR review concept, configure `workflow.external_review.provider` to `github`; and use `issues.listIssues` with filters instead of `projects.*`). Workflow state resolution maps both exact state names and the agent's vocabulary keys (backlog, in_progress, done, etc.) to Linear state types. Team and label lookups are cached per tracker instance.
- **GitLab**: `issues.*` and `labels.*` fully implemented via GitLab REST API v4 (direct `fetch`). Auth via `GITLAB_TOKEN` env var. `review.*` is partially implemented: `pollForReview`, `fetchUnresolvedThreads`, `resolveThread`, and `ciStateOnHead` work via MR discussions and pipeline endpoints; `requestReview` is stubbed (GitLab's approval flow requires admin-configured approval rules). `projects.*` is stubbed (use issues with labels and milestones instead).
- **Jira**: `issues.*` fully implemented via Jira REST API v3 (direct `fetch`). `labels.*` implemented with the Jira-specific semantics that labels are uncontrolled strings per project (no CRUD endpoint): `reconcileLabels` reports an advisory plan against the set of labels currently in use on the project, and `relabelBulk` performs an iterate-and-swap via `relabelIssue` on every matching issue. `review.*` is stubbed (Jira has no native PR surface; point `workflow.external_review.provider` at `github`). `projects.*` is stubbed (Jira's custom-field model is instance-specific). Auth via `JIRA_EMAIL` + `JIRA_API_TOKEN` env vars; the email/token pair is basic-auth'd per Atlassian's 2025 guidance. Rich-text payloads (issue description, comment body) are rendered to Atlassian Document Format (ADF) via `scripts/lib/trackers/jira-adf.mjs`. Jira labels cannot contain whitespace, so the backend rejects label inputs that do.
...
The method list is grouped by namespace on the Tracker interface (see `scripts/lib/trackers/tracker.mjs` for the canonical method names). Each entry is tagged with its current implementation status: **implemented on X** means that backend has working code; **stubbed** means the Tracker throws `NotSupportedError` tagged with `{kind, namespace, op}`.

- **`issues.createIssue`** (implemented on github + linear + gitlab + jira): create the issue and apply labels. GitHub renders the caller-chosen template via `ctx.templateLoader`; Linear, GitLab, and Jira do not support `templateName` (callers must render the body before calling createIssue). Dedupes open issues by exact title match on the repo before creating; returns the existing issue when a match is found. Release-umbrella linking is NOT part of this method on this PR; umbrella-wiring belongs to `release-tracker` when it lands a real backend.
- **`issues.updateIssueStatus`** (implemented on github + linear + gitlab + jira): move an issue to a named status on the tracker's bound Project v2 (GitHub) or per-kind equivalent (Jira workflow transition, Linear workflow state, GitLab open/closed binary state). Never moves an issue to Done: that step is a human gate per `rules/pr-workflow.md`. On Jira, the human-gate check uses the transition's destination `statusCategory` (the `done` category) rather than the literal string "Done", so renamed workflows ("Shipped", "Released") still refuse correctly. No-op when the item is already in the requested status.
- **`issues.comment`** (implemented on github + linear + gitlab + jira): post a comment on an issue (e.g. a filled regression report). The Jira backend renders the body to Atlassian Document Format (ADF) before posting.
- **`issues.relabelIssue`** / **`labels.relabelBulk`** (implemented on github + linear + gitlab + jira; linear's `relabelBulk` uses set-labels-on-issues, not rename-plan, because Linear labels are workspace-scoped): apply a label taxonomy plan from `adapt-system`, including renames via add-new plus remove-old. `relabelIssue` computes a delta against the issue's current labels so calling it twice with the same add/remove set is a no-op on the second run. `labels.relabelBulk` takes a `[{from, to}]` rename plan and walks every open issue carrying `from`, applying `relabelIssue` per issue (delta semantics carry through); failures on individual issues surface on the result without aborting the batch. On Jira, `to === ""` means "delete the label from every matching issue" (Jira has no separate delete-label endpoint).
- **`issues.getIssue`** / **`issues.listIssues`** (implemented on github + linear + gitlab + jira): read-only lookups, respecting depth. `listIssues` paginates through the repo's open-by-default issues and applies client-side label + milestone filters on top of GitHub's `issues(states:)` connection (GraphQL does not expose a label filter on the Repository.issues field). The Jira backend translates the filter set into JQL (`project = X AND statusCategory != Done AND labels in (...)`) and reads via `POST /rest/api/3/search/jql`.
- **`labels.reconcileLabels`** (implemented on github + linear + gitlab + jira): compare the target's labels to a caller-supplied taxonomy (e.g. derived from `ops.config.json -> labels.*`); produce an add / edit / deprecate plan. Dry-run by default; `apply: true` fires create / update / delete against the tracker's native API (GitHub: GraphQL; Linear: GraphQL; GitLab: REST v4). On Jira, `reconcileLabels` is advisory only: Jira has no project-scoped label CRUD, so "create" is a no-op (labels materialise when first used on an issue) and "deprecate" requires an issue-by-issue sweep via `relabelBulk` with `{from, to: ""}`. Idempotent: running twice against the same state produces an empty plan on the second run. `deprecate` is gated behind `allowDeprecate: true` so callers supplying a partial taxonomy don't mass-delete unrelated labels.
- **`projects.reconcileProjectFields`** (implemented on github; stubbed on jira/linear/gitlab): ensure every field in a caller-supplied `declared` list (typically derived from `ops.config.json -> trackers.projects[].fields`) exists on the right Project v2 board; creates missing fields as TEXT via `createProjectV2Field`. Declared names are trimmed and deduplicated at the input boundary. The Project v2 concept is GitHub-specific; other tracker kinds will either surface `NotSupportedError` or map to a native equivalent when they ship.
...
- **`review.requestReview`** / **`review.pollForReview`** / **`review.fetchUnresolvedThreads`** / **`review.resolveThread`** / **`review.ciStateOnHead`** (implemented on github; partially on gitlab (all except requestReview); stubbed on jira/linear): the post-push review iteration surface used by `skills/pr-iteration`. GitHub implements all five via GraphQL. GitLab implements poll, fetch, resolve, and CI via REST; requestReview is stubbed because GitLab requires admin-configured approval rules. Jira and Linear have no native PR surface; configure `workflow.external_review.provider = "github"` when the code lives on GitHub even though tickets live on Jira/Linear.
