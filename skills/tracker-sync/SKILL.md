...
- **`issues.getIssue`** / **`issues.listIssues`** (implemented on github + linear; stubbed on jira): read-only lookups, respecting depth. `listIssues` paginates through the repo's open-by-default issues and applies client-side label + milestone filters on top of GitHub's `issues(states:)` connection (GraphQL does not expose a label filter on the Repository.issues field).
- **`labels.reconcileLabels`** (implemented on github + linear (Linear accepts both `{taxonomy, apply}` and legacy `{desired}` payloads; `apply:false` is dry-run); stubbed on jira): compare the target's labels to a caller-supplied taxonomy (e.g. derived from `ops.config.json -> labels.*`); produce an add / edit / deprecate plan. Dry-run by default; `apply: true` fires `createLabel` / `updateLabel` / `deleteLabel` via GraphQL. Idempotent: running twice against the same state produces an empty plan on the second run. `deprecate` is gated behind `allowDeprecate: true` so callers supplying a partial taxonomy don't mass-delete unrelated labels.
...
- **`review.requestReview`** / **`review.pollForReview`** / **`review.fetchUnresolvedThreads`** / **`review.resolveThread`** / **`review.ciStateOnHead`** (implemented on github; stubbed on jira): the post-push review iteration surface used by `skills/pr-iteration`. GitHub implements all five via GraphQL.
