...
- **Jira**: every namespace stubbed today; every op throws `NotSupportedError`. Callers catch and halt cleanly. Real backend lands in a follow-up PR.
...
The method list is grouped by namespace on the Tracker interface (see `scripts/lib/trackers/tracker.mjs` for the canonical method names). Each entry is tagged with its current implementation status on this PR: **implemented on github** means the github backend has working code; **stubbed on every backend** means every Tracker throws `NotSupportedError` today and the operation is part of the contract, pending a port of the pre-trackers gh-only code path. The real Jira backend lands in a follow-up PR.

- **`issues.createIssue`** (implemented on github + linear + gitlab; stubbed on jira): create the issue and apply labels. GitHub renders the caller-chosen template via `ctx.templateLoader`; Linear and GitLab do not support `templateName` (callers must render the body before calling createIssue). Dedupes open issues by exact title match on the repo before creating; returns the existing issue when a match is found. Release-umbrella linking is NOT part of this method on this PR; umbrella-wiring belongs to `release-tracker` when it lands a real backend.
- **`issues.updateIssueStatus`** (implemented on github + linear + gitlab; stubbed on jira): move an issue to a named status on the tracker's bound Project v2 (GitHub) or per-kind equivalent (Jira workflow state, Linear workflow state, GitLab open/closed binary state). Never moves an issue to Done: that step is a human gate per `rules/pr-workflow.md`. No-op when the item is already in the requested status.
- **`issues.comment`** (implemented on github + linear + gitlab; stubbed on jira): post a comment on an issue (e.g. a filled regression report).
- **`issues.relabelIssue`** / **`labels.relabelBulk`** (both implemented on github + linear (delta semantics on relabelIssue; relabelBulk on Linear uses set-labels-on-issues, not rename-plan, because Linear labels are workspace-scoped); stubbed on jira): apply a label taxonomy plan from `adapt-system`, including renames via add-new plus remove-old. `relabelIssue` computes a delta against the issue's current labels so calling it twice with the same add/remove set is a no-op on the second run. `labels.relabelBulk` takes a `[{from, to}]` rename plan and walks every open issue carrying `from`, applying `relabelIssue` per issue (delta semantics carry through); failures on individual issues surface on the result without aborting the batch.
- **`issues.getIssue`** / **`issues.listIssues`** (implemented on github + linear + gitlab; stubbed on jira): read-only lookups, respecting depth. `listIssues` paginates through the repo's open-by-default issues and applies client-side label + milestone filters on top of GitHub's `issues(states:)` connection (GraphQL does not expose a label filter on the Repository.issues field).
- **`labels.reconcileLabels`** (implemented on github + linear + gitlab; stubbed on jira): compare the target's labels to a caller-supplied taxonomy (e.g. derived from `ops.config.json -> labels.*`); produce an add / edit / deprecate plan. Dry-run by default; `apply: true` fires create / update / delete against the tracker's native API (GitHub: GraphQL; Linear: GraphQL; GitLab: REST v4). Idempotent: running twice against the same state produces an empty plan on the second run. `deprecate` is gated behind `allowDeprecate: true` so callers supplying a partial taxonomy don't mass-delete unrelated labels.
- **`projects.reconcileProjectFields`** (implemented on github; stubbed on jira): ensure every field in a caller-supplied `declared` list (typically derived from `ops.config.json -> trackers.projects[].fields`) exists on the right Project v2 board; creates missing fields as TEXT via `createProjectV2Field`. Declared names are trimmed and deduplicated at the input boundary. The Project v2 concept is GitHub-specific; other tracker kinds will either surface `NotSupportedError` or map to a native equivalent when they ship.
- **`projects.listProjectItems`** (implemented on github; stubbed on jira): paginated read-only snapshot of Project v2 items with their field values (text / number / date / single-select / iteration). Hard-capped at 2000 items; fails loud when an item has >50 field values (refuses to return truncated data).
- **`projects.updateProjectField`** (implemented on github; stubbed on jira): single-field update on a project item. Refuses to write to a field not declared in `trackers.projects[].fields` (except the implicit `status_field`). Input-boundary validation on the `value` shape runs BEFORE any network call.
- **`review.requestReview`** / **`review.pollForReview`** / **`review.fetchUnresolvedThreads`** / **`review.resolveThread`** / **`review.ciStateOnHead`** (implemented on github; partially on gitlab (all except requestReview); stubbed on jira/linear): the post-push review iteration surface used by `skills/pr-iteration`. GitHub implements all five via GraphQL. GitLab implements poll, fetch, resolve, and CI via REST; requestReview is stubbed because GitLab requires admin-configured approval rules.
