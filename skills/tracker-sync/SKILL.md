...
- **Linear**: `issues.*` and `labels.*` fully implemented via Linear's GraphQL API (direct `fetch`, no `@linear/sdk`). Auth via `LINEAR_API_KEY` env var. `review.*` and `projects.*` stubbed with pointed messages ("Linear has no native PR review concept; configure `workflow.external_review.provider: "github"`" and "use `issues.listIssues` with filters instead"). Workflow state resolution maps both exact state names and the agent's vocabulary keys (backlog, in_progress, done, etc.) to Linear state types. Team and label lookups are cached per tracker instance.
